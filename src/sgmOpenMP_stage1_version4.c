...
// sgmOpenMP — Version 4: all four steps run in parallel.
// determine_costs, iterate_direction, inplace_sum_views, and create_disparity_view
// are all parallelised. This is the complete OpenMP implementation.
                ...
                const int w, const int h, const int disp_range )
{
    ...
    for (int j = 0; j < ny; j++) {
      ...
      for (dirx = -1; dirx < 2; dirx++) {
          ...
          // Each element k is independent; no write overlaps between threads.
          #pragma omp parallel for
          for (int k = 0; k < total_elems; k++)
              total_accumulated_costs[k] += dir_accumulated_costs[k];
      }
    ...
    // Version 4: create_disparity_view parallelised. Each pixel selects the disparity
    // with the lowest accumulated cost; no dependency between (i,j) pairs.
    // The alias satisfies the ACCUMULATED_COSTS macro.
    {
        const int *accumulated_costs = total_accumulated_costs;

        // Each pixel finds its own minimum independently; no dependency between (i,j) pairs.
        #pragma omp parallel for
        for (int j = 0; j < ny; j++) {
            for (int i = 0; i < nx; i++) {
