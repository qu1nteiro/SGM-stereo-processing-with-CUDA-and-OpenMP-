 ...
 *
 * Block size: (8, 32)
 *   blockDim.y = 32 = disp_range exactly. The disparity dimension is
 *   covered by exactly one block row — no partial blocks, no wasted
 *   threads. More importantly, the 32 d-threads in a warp write to
 *   costs[i*disp_range + j*nx*disp_range + d] with d = 0..31, which
 *   are consecutive memory addresses — a single coalesced 128-byte
 *   global memory transaction serves the entire warp.
 * ------------------------------------------------------------ */
...
{
    /* One left-image pixel per j-lane in this block (blockDim.x = 8) */
    __shared__ int s_left[8];
                ...
                const int w, const int h, const int disp_range )
{
    ...
    /* ----------------------------------------------------------------
     * Step 1 - determine_costs (GPU, shared memory, block 8×32)
     *
     * Block dimensions chosen to match the data layout:
     *   blockDim.x = 8  → j (rows): 8 per block, s_left has 8 entries
     *   blockDim.y = 32 → d (disparities): matches disp_range exactly,
     *                     so grid.y = ceil(32/32) = 1 — no partial blocks.
     *
     * The 32 d-threads per warp write to consecutive addresses in d_costs
     * (d is the fastest-varying index), producing a single coalesced
     * 128-byte global memory transaction per warp per column i.
     * ---------------------------------------------------------------- */
    {
        int threads = 256;
     ...
     * than the cost-volume copies we eliminated in earlier steps.
     * ---------------------------------------------------------------- */
    pixel_t *d_disp = NULL;
