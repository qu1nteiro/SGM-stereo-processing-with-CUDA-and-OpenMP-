...
// sgmOpenMP — Version 3: determine_costs, iterate_direction, and inplace_sum_views
// run in parallel. The sum loop is converted from pointer-walking to an indexed
// for loop so OpenMP can partition the index range across threads.
// create_disparity_view still runs serially.
