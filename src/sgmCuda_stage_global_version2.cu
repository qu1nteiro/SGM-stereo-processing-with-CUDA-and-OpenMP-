                ...
                const int w, const int h, const int disp_range )
{
    ...
    /* ----------------------------------------------------------------
     * Step 2 - iterate_direction (GPU)
     *
     * For each of the four scanning directions we:
     *   1. Reset d_dir_accumulated to zero.
     *   2. Launch the direction kernel (one thread per row or column).
     *   3. Copy the result back to the host.
     *   4. Accumulate into h_total_accumulated on the CPU.
     *
     * Steps 3-4 are temporary: Step 3 will replace the CPU sum with
     * a GPU kernel, removing this round-trip entirely.
     *
     * Grid sizing:
     *   Horizontal kernels: one thread per row  → grid covers ny rows.
     *   Vertical   kernels: one thread per col  → grid covers nx cols.
     * ---------------------------------------------------------------- */

    /* Host-side buffers for the temporary Step 2 → CPU handoff */
    int *h_dir_accumulated   = (int *) malloc(total_elems * sizeof(int));
    int *h_total_accumulated = (int *) calloc(total_elems, sizeof(int));
    if (!h_dir_accumulated || !h_total_accumulated) {
    ...
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaMemcpy(h_dir_accumulated, d_dir_accumulated, total_elems * sizeof(int), cudaMemcpyDeviceToHost));
    inplace_sum_views(h_total_accumulated, h_dir_accumulated, nx, ny, disp_range);
