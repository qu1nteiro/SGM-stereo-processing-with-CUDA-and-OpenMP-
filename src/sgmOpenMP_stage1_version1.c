...
// sgmOpenMP — Version 1: only determine_costs runs in parallel.
// The path integration steps (iterate_direction, inplace_sum_views, create_disparity_view)
// still run serially using the same host helper functions as sgmHost.
// Each costs[i][j][d] write is independent — no row, disparity or column
// depends on another, so parallelising the j-loop is race-free.
                ...
                const int w, const int h, const int disp_range )
{
    ...
    for(dirx=-1; dirx<2; dirx++) {
        if(dirx==0 && diry==0) continue;
        std::fill(dir_accumulated_costs, dir_accumulated_costs+nx*ny*disp_range, 0);
        iterate_direction( dirx,diry, left_image, costs, dir_accumulated_costs, nx, ny, disp_range);
        inplace_sum_views( accumulated_costs, dir_accumulated_costs, nx, ny, disp_range);
    }
    dirx=0;
    for(diry=-1; diry<2; diry++) {
    ...
    create_disparity_view(accumulated_costs, disp_image, nx, ny, disp_range );
